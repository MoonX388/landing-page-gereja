  ...
  return (
    ...
    <header className="bg-background border-b border-border sticky top-0 z-40">
      <div className="flex items-center justify-between px-4 py-3 md:px-6">
        ...
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="md:hidden text-muted-foreground p-1 rounded-lg hover:bg-accent hover:text-accent-foreground">
          {sidebarOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
        </button>
        
        <div className="flex items-center gap-3 ml-auto relative" ref={dropdownRef}>
          <button onClick={() => setProfileDropdownOpen(!profileDropdownOpen)} className="flex items-center gap-2 p-1.5 rounded-xl hover:bg-accent hover:text-accent-foreground text-left transition-colors focus:outline-none">
            ...
            <div className="absolute right-0 top-full mt-2 w-56 rounded-xl bg-popover text-popover-foreground border border-border shadow-xl py-2 z-50">
              ...
              <div className="p-1 space-y-0.5">
                <button 
                  ...
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm hover:bg-accent hover:text-accent-foreground transition-colors"
                >
